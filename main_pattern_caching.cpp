...
private:
  ...
  struct CompatibilityCache {
    ...
    void initialize(size_t pattern_count) {
      cache.resize(pattern_count,
                   std::vector<std::vector<bool>>(pattern_count,
  ...
  void propagate(int x, int y) {
    ...
    std::vector<std::vector<bool>> changed(height,
                                           std::vector<bool>(width, false));
    changed[y][x] = true;
      ...
      for (int dy = -1; dy <= 1; dy++) {
        for (int dx = -1; dx <= 1; dx++) {
          ...
          for (const Pattern &p2 : neighbor_patterns) {
            ...
            // Check if any current pattern is compatible with this neighbor
            // pattern
  ...
  std::pair<int, int> findLowestEntropy() {
    ...
    if (candidates.empty())
      return {-1, -1};
    return candidates[std::uniform_int_distribution<>(0, candidates.size() -
                                                             1)(rng)];
  }
    ...
    for (int dy = -1; dy <= 1; dy++) {
      for (int dx = -1; dx <= 1; dx++) {
        ...
        if (wave[ny][nx].empty()) {
          ...
          // If still empty after repair attempt, restore state and try next
          // neighbor
          if (wave[ny][nx].empty()) {
            if (!restoreState()) {
              return false;
