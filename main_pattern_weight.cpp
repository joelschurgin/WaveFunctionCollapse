...
int main() {
    // Read the Flowers image
    int flowers_width, flowers_height, flowers_channels;
    unsigned char* flowers_data = stbi_load("Images/Flowers.png", &flowers_width, &flowers_height, &flowers_channels, 4);
    
    if (!flowers_data) {
        std::cerr << "Failed to load Flowers.png: " << stbi_failure_reason() << std::endl;
        return 1;
    }
