...
private:
    ...
    struct CompatibilityCache {
        ...
        void initialize(size_t pattern_count) {
            cache.resize(pattern_count, std::vector<std::vector<bool>>(pattern_count, std::vector<bool>(8, false)));
        }
    } compatibility_cache;
    ...
    void extractPatterns(const unsigned char* input, int input_width, int input_height) {
                    ...
                    for (int dx = 0; dx < N; dx++) {
                        for (int c = 0; c < channels; c++) {
                            pattern.data[(dy * N + dx) * channels + c] = 
                                input[((y + dy) * input_width + (x + dx)) * channels + c];
                        }
                    }
                }
            ...
            for (size_t j = 0; j < patterns.size(); j++) {
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        ...
                        compatibility_cache.cache[i][j][dir] = isCompatible(patterns[i], patterns[j], dx, dy);
                    }
                }
            }
        }
    ...
    void propagate(int x, int y) {
        ...
        // Queue to track cells that need to be processed
        ...
        std::vector<std::vector<bool>> queued(height, std::vector<bool>(width, false));
        queued[y][x] = true;
        
        // Keep track of which cells have changed in the current iteration
        std::vector<std::vector<bool>> changed(height, std::vector<bool>(width, false));
        changed[y][x] = true;
            ...
            for (int dy = -1; dy <= 1; dy++) {
                for (int dx = -1; dx <= 1; dx++) {
                    ...
                    for (const Pattern& p2 : neighbor_patterns) {
                        ...
                        // Check if any current pattern is compatible with this neighbor pattern
    ...
    std::pair<int, int> findLowestEntropy() {
        ...
        return candidates[std::uniform_int_distribution<>(0, candidates.size() - 1)(rng)];
    }
    ...
    bool tryLocalRepair(int x, int y) {
        ...
        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                ...
                if (wave[ny][nx].empty()) {
                    wave[ny][nx] = std::set<Pattern>(patterns.begin(), patterns.end());
                    updateEntropyCache(nx, ny);
                    propagate(nx, ny);
                    
                    // If still empty after repair attempt, restore state and try next neighbor
                    if (wave[ny][nx].empty()) {
                        if (!restoreState()) {
                            return false;
                        }
...
public:
    WaveFunctionCollapse(int w, int h, int c, unsigned int seed = std::random_device{}()) 
    ...
    void initialize(const unsigned char* input, int input_width, int input_height) {
        extractPatterns(input, input_width, input_height);
        
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                wave[y][x] = std::set<Pattern>(patterns.begin(), patterns.end());
    ...
    bool generate() {
        ...
        while (true) {
            ...
            std::vector<Pattern> possible_patterns(wave[y][x].begin(), wave[y][x].end());
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (wave[y][x].empty()) {
                    ...
                    for (int c = 0; c < channels; c++) {
                        output[((height - 1 - y) * width + x) * channels + c] = (c == 0) ? 255 : 0; // Red for error
                    }
                    continue;
                }
                ...
                for (int c = 0; c < channels; c++) {
                    output[((height - 1 - y) * width + x) * channels + c] = p.data[c];
                }
            }
        }
    }

    bool hasEmptyWave(int x, int y) {
        return wave[y][x].empty();
    }
};
...
GenerationResult tryGeneration(const unsigned char* input, int input_width, int input_height, 
                              int width, int height, int channels, unsigned int seed) {
...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " <input_image> --size=<number>" << std::endl;
        return 1;
    }
    ...
    if (size_arg.substr(0, 7) != "--size=") {
        std::cerr << "Error: Second argument must be in format --size=<number>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        std::cerr << "Error: Invalid size value. Must be a positive number." << std::endl;
        return 1;
    }

    // Read the input image
    int flowers_width, flowers_height, flowers_channels;
    unsigned char* flowers_data = stbi_load(input_filename.c_str(), &flowers_width, &flowers_height, &flowers_channels, 4);
    
    if (!flowers_data) {
        std::cerr << "Failed to load " << input_filename << ": " << stbi_failure_reason() << std::endl;
        return 1;
    }
    ...
    const int MAX_ATTEMPTS = 1;  // Maximum number of attempts with different seeds
    ...
    for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        unsigned int seed = 12345 + attempt;
        GenerationResult result = tryGeneration(
            flowers_data, flowers_width, flowers_height,
    ...
    if (stbi_write_png("output.png", width, height, channels, best_result.output.data(), width * channels)) {
